...
namespace RazorPagesDemo.Pages
{
    public class ContactMeModel : PageModel
    {
        ...
        public void OnPostSendMessage()
        {
            ...
            {
                ModelState.AddModelError(nameof(ContactName), "Cantact Name must contain 5 or more characters");
            }
            if (string.IsNullOrWhiteSpace(ContactEmail))
            {
                ModelState.AddModelError(nameof(ContactEmail), "Contact Email is required and cannot be blank");
            }
            //else if (Regex.Match(ContactEmail, "").)
            if (string.IsNullOrWhiteSpace(nameof(ContactComments)))
            {
                ModelState.AddModelError(nameof(ContactComments), "Contac Comments is required and cannot be blank.");
            }
